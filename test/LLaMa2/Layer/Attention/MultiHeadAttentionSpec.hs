...
spec = do
    ...
    describe "multiHeadAttentionStage - Sequential Tokens" $ do
        ...
        context "multiHeadAttentionStage - Back-to-back tokens (zero gap)" $ do
            it "resets accumulator, FSM, and Q/K/V between tokens" $ do
                let maxCycles = 400
                    ...
                    -- Same simple test weights as earlier tests
                    testRow  = (repeat 1, 0) :: RowI8E ModelDimension
                    testRow' = (repeat 1, 0) :: RowI8E HeadDimension
                    testQMatrix = repeat testRow
                    testKMatrix = repeat testRow
                    testVMatrix = repeat testRow
                    ...
                    -- SAFE fixed second-pulse cycle: avoid probing the DUT to compute it.
                    -- If your MHA really needs more cycles, raise this constant.
                    secondPulseCycle :: Int
                    secondPulseCycle = 80
                    token2Start = secondPulseCycle

                    -- Build VALID stream: pulse at cycle 1, and pulse at secondPulseCycle
                    validStream =
                        [ i == 1 || i == secondPulseCycle
                        | i <- [0 .. maxCycles - 1]
                        ]
                    validIn = fromList validStream

                    -- Sequence positions: 0 for token 1 until secondPulseCycle-1, then 1
                    seqPos = fromList $ P.replicate secondPulseCycle (0 :: Index 512)
                                    P.++ P.replicate (maxCycles - secondPulseCycle) 1

                    -- Run the simulation (single exposeClockResetEnable; break combinational arvalid loop)
                    (masterOut, xAfterAttn, _q, _k, _v, qkvReady, _qkvDone, _writeDone, attentionDone, debugInfo) =
                        exposeClockResetEnable
                            (let arvalidSignal' = register False (Master.arvalid masterOut)
                            in multiHeadAttentionStage
                                    (mockDRAM arvalidSignal')
                                    0 mhaParams seqPos layerData validIn)
                            CS.systemClockGen CS.resetGen CS.enableGen

                    -- Sample signals
                    debugStates   = P.take maxCycles $ sample $ qhState debugInfo
                    debugRowResS  = P.take maxCycles $ sample $ qhRowResult debugInfo

                    debugResets = P.take maxCycles $ sample $ qhRowReset debugInfo
                    debugEnables = P.take maxCycles $ sample $ qhRowEnable debugInfo
                    debugAccums = P.take maxCycles $ sample $ qhAccumValue debugInfo
                    debugQOuts = P.take maxCycles $ sample $ qhQOut debugInfo

                    qkvReadyS = P.take maxCycles $ sample qkvReady
                    attnDoneS = P.take maxCycles $ sample attentionDone
                    xAfterAttnS = P.take maxCycles $ sample xAfterAttn

                    -- Find completion cycles
                    attnDoneIndices = DL.findIndices id attnDoneS
                    firstDone = if P.null attnDoneIndices then error "First token never finished" else P.head attnDoneIndices
                    secondDone = if P.length attnDoneIndices < 2
                                then error "Second token never finished"
                                else attnDoneIndices P.!! 1

                    -- Defensive index choices: assert FSM/ready *before* secondPulseCycle
                    idxBeforeSecond = max 0 (secondPulseCycle - 1)
                    -- extract debug info at specific cycles (safe indices)
                    stateBefore2 = debugStates P.!! idxBeforeSecond
                    rowResAt2    = debugRowResS P.!! secondPulseCycle

                    
                    token1Done = if not (DL.null attnDoneIndices)
                                    then DL.head attnDoneIndices else 0

                
                -- Print full diagnostic at token 2 start
                putStrLn $ "\n=== Token 1 completion (cycle " P.++ show token1Done P.++ ") ==="
                putStrLn $ "State: " P.++ show (debugStates P.!! token1Done)
                putStrLn $ "QOut: " P.++ show (toList $ debugQOuts P.!! token1Done)
                
                putStrLn $ "\n=== Token 2 start (cycle " P.++ show token2Start P.++ ") ==="
                putStrLn $ "State: " P.++ show (debugStates P.!! token2Start)
                putStrLn $ "Reset: " P.++ show (debugResets P.!! token2Start)
                putStrLn $ "Enable: " P.++ show (debugEnables P.!! token2Start)
                putStrLn $ "Accum: " P.++ show (debugAccums P.!! token2Start)
                putStrLn $ "QOut: " P.++ show (toList $ debugQOuts P.!! token2Start)

                -- ASSERTIONS
                -- 1. FSM must be idle before accepting next token (we check just before second pulse)
                stateBefore2 `shouldBe` MIdle

                -- 2. Accumulator must reset to zero at start of token 2
                rowResAt2 `shouldBe` 0

                -- 3. QKV ready must be True BEFORE token 2 validIn (check the cycle before second pulse)
                qkvReadyS P.!! idxBeforeSecond `shouldBe` True

                -- 4. MHA must emit second attentionDone
                secondDone `shouldSatisfy` (< maxCycles - 1)

                -- 5. Token 1 and Token 2 results must MATCH, because input vectors are identical
                let out1 = xAfterAttnS P.!! firstDone
                    out2 = xAfterAttnS P.!! secondDone
                    differences = P.zipWith (\a b -> abs (a - b)) (toList out1) (toList out2)
                    allClose = P.all (< 1e-3) differences
                allClose `shouldBe` True
