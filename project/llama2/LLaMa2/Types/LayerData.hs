...
-- | Simplified cycle stages - attention sub-stages now internal to MultiHeadAttention
data CycleStage = Stage_Attention     -- Handles QKV projection, KV cache write, and attention internally
    | Stage_FeedForward   -- FFN and residual
    | Stage_Classifier    -- Final output projection (only after all layers)
