-- File: LLaMa2/Layer/Attention/QKVProjection.hs
...
--------------------------------------------------------------------------------
-- Q head projector with DDR streaming (FIXED: Latch + Handshake)
--------------------------------------------------------------------------------
...
queryHeadProjector dramSlaveIn layerIdx headIdx inputValid downStreamReady stepCount xHat params =
 ...
 where
  ...
  -- FIX 1: We use the fetchTrigger from the FSM to initiate the AXI read
  ...
  nextRowIndex = mux (state .==. pure OPS.MIdle)
                      (pure 0)
...
--------------------------------------------------------------------------------
-- QKV Projection Controller (FIXED: DEADLOCK REMOVED)
--------------------------------------------------------------------------------
qkvProjectionController ::
  HiddenClockResetEnable dom
  => Slave.AxiSlaveIn dom
...
qkvProjectionController dramSlaveIn layerIdx inputValid downStreamReady input params seqPos =
 ...
 where
  ...
  -- FIX 4: Do NOT gate enableRaw with projReadyOut_d. 
  -- We must sustain the Enable signal even when the Projector goes busy (ready=False).
  ...
  -- We still use projReadyOut_d to tell the upstream FSM to wait.
