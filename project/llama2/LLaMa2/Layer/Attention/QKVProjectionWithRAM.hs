module LLaMa2.Layer.Attention.QKVProjectionWithRAM
...
--------------------------------------------------------------------------------
keyValueHeadProjectorWithRAM :: forall dom.
  HiddenClockResetEnable dom
  => Signal dom Bool                          -- ^ validIn
  -> Signal dom Bool                          -- ^ readyIn
  -> SingleHeadComponentQ                     -- ^ hardcoded (fallback)
...
keyValueHeadProjectorWithRAM validIn _readyIn headComp stepCountSig xHatSig ramK ramV useRAM =
...
qkvProjectorWithRAM validIn readyIn mhaQ seqPosSig xSig weightBuffer useRAM =
  ...
  qResults = imap qHead (headsQ mhaQ)
   where
    ...
    qHead hIx headQ =
      ...
      in queryHeadProjectorWithRAM validIn readyIn headQ seqPosSig xNorm ramQ useRAM
  ...
  kvResults = imap kvHead kvHeadIndices
   where
    ...
    kvHead kvIx qIx =
      ...
      in keyValueHeadProjectorWithRAM validIn readyIn headQ seqPosSig xNorm ramK ramV useRAM
...
qkvProjectionControllerWithRAM ::
  ...
  -> ( Signal dom ( Vec NumQueryHeads    (Vec HeadDimension FixedPoint)
     ...
     , Signal dom Bool
     , Signal dom Bool )
qkvProjectionControllerWithRAM inValid outReady input mhaQ psSig weightBuf useRAM =
