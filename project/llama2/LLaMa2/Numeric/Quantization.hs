...
ln2InvF :: FixedPoint
ln2InvF = 1.4426950408889634  -- 1/ln(2), direct SFixed literal (no Double intermediate)
...
exp2FracLUT = $(listToVecTH
  [ realToFrac (2.0 ** (fromIntegral (k :: Int) / 256.0) :: Double) :: FixedPoint
  | k <- [0..255 :: Int] ])

-- 2^f with f in [0,1); uses asyncRom (Clash primitive) to avoid mux-tree expansion.
{-# NOINLINE exp2Frac #-}
