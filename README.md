...
Output Verilog is written to `/tmp/clash-verilog-<model>/`, one subdirectory
per block. Per-block logs (`clash-1-sampler.log`, etc.) are kept there for
inspection.

The hierarchical mode synthesises five blocks independently, bottom-up:
...
**Memory:** Clash normalization is RAM-intensive for large models. `model-nano`
synthesises comfortably on a 32 GB machine. All variants up to and including
`model-7b` have been validated through full Verilog generation. Use hierarchical
mode if a full synthesis run runs out of memory — it isolates which block is
the bottleneck.

**Downstream EDA is out of scope for this repository.** FPGA implementation
(LUT/FF/BRAM resource counts, timing closure, place-and-route) requires
running the generated Verilog through a vendor tool such as Vivado (Xilinx/AMD)
or Quartus (Intel/Altera). The design targets 400 MHz with an estimated
~512 DSP blocks as noted in PRODUCT.md.
